...
describe("Richieste corrette a /games/catenaFinale ", function () {
    test("Data nel passato", async function () {
        ...
        nock("https://api.twitter.com")
            ...
            .reply(200, generateTweets(100, true, date.startOf("day").toISOString(), date.endOf("day").toISOString(), "#reazioneacatena cammelletto"));
        const res = await curr_session.get("/games/catenaFinale").query({ date: date.startOf("day").toISOString() }).expect(200);
    ...
    test("Data nel passato con risultato contenente URL", async function () {
        ...
        nock("https://api.twitter.com")
            .get('/2/tweets/search/all').query(generateParams(query, "", 100, date.startOf("day").toISOString(), date.endOf("day").toISOString()))
            .reply(200, generateTweets(100, true, date.startOf("day").toISOString(), date.endOf("day").toISOString(), "#reazioneacatena https://t.co/LS655lKPU3"));
        const res = await curr_session.get("/games/catenaFinale").query({ date: date.startOf("day").toISOString() }).expect(200);
    ...
    test("Data nel futuro", async function () {
        ...
        nock("https://api.twitter.com")
            .get('/2/tweets/search/all').query(generateParams(query, "", 100, date.startOf("day").toISOString(), date.endOf("day").toISOString()))
            .reply(400);
        const res = await curr_session.get("/games/catenaFinale").query({ date: date.startOf("day").toISOString() }).expect(400);
