...
async function multipleTweetsFetch(fetcher, query, pagination_token="", quantity=10) {
    ...
    while (quantity > 0) {
        try {
            ...
            const tweets = await fetcher(query, pagination_token, quantity < 100 ? quantity : 100);         // Viene richiamato il fetcher con query, pagination_token e quantità. La quantità è 100 se rimangono ancora più di 100 tweet da ricercare.
